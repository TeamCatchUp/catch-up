...
export function TeamSpaceDropDownContent({ teamSpaces, selectedId, onSelect }: TeamSpaceDropDownContentProps) {
  return (
    ...
    >
          ...
          return (
            <PopoverClose asChild key={team.id}>
              <button
                ...
                className={cn(
                  'hover:bg-fill-normal-interaction-hover flex h-10 cursor-pointer items-center gap-2.5 rounded-lg p-2 transition-colors',
                  isSelected && 'bg-fill-normal-interaction-hover',
                )}
              >
                <ArrowRight className="text-icon-normal-normal h-6 w-6" />
                <span className={cn('text-body-small text-text-normal-normal', isSelected && 'font-semibold')}>
                  {team.name}
                </span>
              </button>
            </PopoverClose>
      ...
      {/* divider */}
      <div className="bg-line-normal-neutral h-px w-full" />

      {/* 팀스페이스 추가 */}
      <button className="hover:bg-fill-normal-interaction-hover flex w-full cursor-pointer items-center gap-2.5 rounded-lg p-2 transition-colors">
        <AddHome className="text-icon-normal-normal h-6 w-6" />
        <span className="text-body-small text-text-normal-normal">팀스페이스 추가</span>
      </button>
    </PopoverContent>
  );
}
