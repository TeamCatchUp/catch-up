...
export const SearchOptionButton = forwardRef<HTMLButtonElement, SearchOptionButtonProps>(
  ({ Icon, label, selected = false, onClick, className, ...props }, ref) => {
    return (
      ...
      >
        <Icon className={cn('h-5 w-5 shrink-0', selected ? 'text-blue-55' : 'text-gray-70')} />
        <div className={cn('text-body-small truncate whitespace-nowrap', selected ? 'text-blue-55' : 'text-gray-80')}>
          {label}
        </div>
      </button>
    );
...
export const SearchOptionDisabledButton = ({ label, Icon }: DisabledButtonProps) => {
  return (
    ...
    >
      <Icon className="text-gray-30 size-5 shrink-0" />
