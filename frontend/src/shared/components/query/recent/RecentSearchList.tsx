  ...
  if (!querys || querys.length === 0)
    return (
      <div className="flex w-full flex-col gap-2.5">
        <span className="text-body-xsmall px-1.5 font-medium text-content-alternative">{title}</span>
      ...
      <div className="flex flex-col gap-1">
        {querys.slice(0, 5).map((item, index) => (
          <Link
            ...
            className="hover:bg-fill-interaction-hover group flex h-10 w-full cursor-pointer items-center gap-2 rounded-xl bg-fill-normal px-2 py-1 transition-colors"
          >
            <div className="rounded-rounded border-edge-neutral bg-fill-strong flex shrink-0 items-center justify-center gap-2.5 border p-1.5">
              <ChatIcon className="h-5 w-5 text-icon-neutral" />
            </div>
