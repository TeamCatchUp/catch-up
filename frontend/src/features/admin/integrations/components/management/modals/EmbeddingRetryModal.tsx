...
const EmbeddingRetryModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={isLoading ? undefined : onOpenChange}>
      <DialogContent
        hideClose
        className="border-line-normal-normal bg-fill-normal-normal shadow-modal flex w-140 flex-col gap-4 rounded-3xl border p-0 py-5"
      >
        <div className="flex h-9 items-center gap-3 px-6">
          <DialogTitle className="text-heading-large text-text-normal-normal flex-1">임베딩 재시도</DialogTitle>
          {!isLoading && (
            <button
              type="button"
            ...
            >
              <Cancel className="text-text-normal-normal size-6" />
            </button>
          )}
        </div>

        {isLoading ? (
          <div className="border-line-normal-assistive flex flex-col gap-2 border-t px-6 pt-6">
            <h3 className="text-heading-medium text-text-normal-normal">
              다시 임베딩을 진행하고 있어요. 잠시만 기다려 주세요:)
            </h3>
            <p className="text-body-small text-text-normal-neutral">재시도가 완료되면 창이 바로 꺼집니다.</p>
          </div>
        ) : (
          <>
            <div className="border-line-normal-assistive flex flex-col gap-6 overflow-x-clip overflow-y-auto border-t px-6 pt-6">
              <div className="flex flex-col gap-2.5">
                <div className="flex items-center gap-5">
                  <h3 className="text-heading-medium text-text-normal-normal flex-1">임베딩 현황</h3>
                  <span className="text-label-small text-text-normal-alternative max-w-100">
                ...
                {!isFirstRetry && (
                  <div className="bg-fill-normal-strong flex items-center gap-3 rounded-xl p-3">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-body-small text-text-normal-normal">임베딩 재시도</span>
                      <span className="text-body-small text-text-normal-alternative">
                        {retryingCount.toLocaleString()}개
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  <div className="bg-fill-normal-strong flex flex-1 items-center gap-3 rounded-xl p-3">
                    <div className="bg-fill-normal-normal flex shrink-0 items-center justify-center rounded-xl p-2">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-label-small text-text-normal-neutral">임베딩 제안</span>
                {isFirstRetry ? (
                  <>
                    <h3 className="text-heading-medium text-text-normal-normal">
                      실패한 {failedCount.toLocaleString()}개만 다시 해볼까요?
                    </h3>
                    <p className="text-body-small text-text-normal-neutral">
            ...
            <div className="flex h-9 items-start justify-end gap-3 px-6">
              <button
                type="button"
                onClick={() => onOpenChange(false)}
                className="border-line-normal-normal bg-fill-normal-normal text-body-small text-text-normal-normal flex h-9 cursor-pointer items-center justify-center overflow-clip rounded-full border px-3 py-1.5"
              >
                취소
              </button>
              <button
