...
import { CONFLUENCE_GUIDE_IMAGES } from '../../../constants/guideImages';
...
export default function ConfluenceGuideSection() {
  return (
    <div className="border-line-normal-neutral bg-fill-normal-normal flex flex-col gap-10 overflow-clip rounded-xl border p-6">
      ...
      <div className="flex flex-col gap-2">
        <h4 className="text-heading-medium text-text-normal-normal">1. Atlassian 사이트 연결 및 권한 승인</h4>
        <p className="text-label-small text-text-normal-normal">
        ...
        <button
          ...
          className="border-line-normal-normal flex cursor-pointer items-start gap-4 rounded-xl border px-3 py-2 text-left"
        >
          <div className="bg-fill-normal-normal flex size-10 shrink-0 items-center justify-center">
            <IconAT className="size-10" />
          </div>
          <div className="flex min-w-0 flex-col">
            <span className="text-body-xsmall text-text-normal-normal">Atlassian account</span>
            <span className="text-label-xsmall text-text-normal-alternative truncate">{ATLASSIAN_PROFILE_URL}</span>
          </div>
        </button>
      </div>
      ...
      {/* 3. 문서의 지식화 */}
      <div className="flex flex-col gap-2">
        <h4 className="text-heading-medium text-text-normal-normal">3. 문서의 지식화</h4>
        <p className="text-label-small text-text-normal-normal">
      ...
      {/* 클로징 */}
      <p className="text-label-small text-text-normal-strong">팀이 커져도, 기획의 맥락은 그대로 남습니다.</p>
    </div>
  );
}
