...
// /hybrid-search 의 URL state(?q&tools&start&end&smart_filter) 파싱·갱신 hook.
...
export function useHybridSearchUrlState(): HybridSearchUrlState {
  ...
  const smartFilter = searchParams.get('smart_filter') !== 'false';
  ...
  const commitSearch = useCallback(
    (nextKeyword: string, nextTools: ToolFilter[], nextRange: DateRange | undefined, nextSmartFilter: boolean) => {
      ...
      next.set('smart_filter', String(nextSmartFilter));
  ...
  return { keyword, tools, smartFilter, dateRange, commitSearch };
}
