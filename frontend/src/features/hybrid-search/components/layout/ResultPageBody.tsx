...
// Body shell: px-16, inner max width 1420px, gap 24px.
...
export default function ResultPageBody({ children, side }: PropsWithChildren<ResultPageBodyProps>) {
  return (
    <section className="flex min-h-0 flex-1 justify-center overflow-hidden px-16">
      <div className="flex h-full min-h-0 w-full max-w-[1420px] items-stretch gap-6 overflow-hidden">
        ...
        <aside className="custom-scrollbar border-edge-normal min-h-0 w-105.75 shrink-0 overflow-y-auto overscroll-contain border-l">
          {side}
        </aside>
      </div>
    </section>
