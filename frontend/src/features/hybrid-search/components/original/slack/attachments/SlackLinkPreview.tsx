...
export default function SlackLinkPreview({ attachment }: SlackLinkPreviewProps) {
  ...
  return (
    <div className="bg-fill-normal-assistive-dark border-line-normal-neutral flex w-70 flex-col items-start justify-center overflow-hidden rounded-xl border">
      {imageUrl && (
      ...
      <div className="flex w-full flex-col items-start gap-1 px-4 py-5">
        {attachment.title && (
          <p className="text-heading-small text-text-normal-neutral max-w-62.5 truncate font-semibold">
            {attachment.title}
          </p>
        )}
        {attachment.text && (
          <p className="text-body-small text-text-normal-neutral w-full truncate font-medium">{attachment.text}</p>
        )}
        {href && (
          <a
            href={href}
            ...
            className="text-body-xsmall text-text-primary-normal w-full truncate underline"
          >
            링크 {href}
          </a>
        )}
