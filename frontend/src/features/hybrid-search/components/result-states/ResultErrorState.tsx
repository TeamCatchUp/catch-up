...
export default function ResultErrorState({ onRetry }: ResultErrorStateProps) {
  return (
    ...
    >
      <p className="text-body-small text-text-normal-alternative">검색 중 오류가 발생했어요. 다시 시도해주세요.</p>
      <button
        type="button"
        onClick={onRetry}
        className="text-body-small text-text-primary-normal hover:bg-fill-primary-normal-interaction-hover-assistive cursor-pointer rounded-full px-3 py-1.5 font-medium transition-colors"
      >
        다시 시도
      </button>
    </motion.div>
