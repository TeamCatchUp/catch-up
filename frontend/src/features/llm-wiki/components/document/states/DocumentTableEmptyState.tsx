...
/** 표에 보여줄 문서가 없을 때 행 자리에 들어가는 안내. 표 헤더는 그대로 남는다. */
export default function DocumentTableEmptyState() {
  return (
    <div className="flex w-full flex-col items-center gap-5 py-45">
      <IconEmptyDocument aria-hidden className="h-13.75 w-16 shrink-0" />
      <p className="text-body-xsmall text-text-normal-assistive text-center">문서가 없어요</p>
    </div>
  );
}
