...
function ScheduleTriggerField({ field, onOpenRequest }: ScheduleTriggerFieldProps) {
  return (
    <div className="flex min-w-0 flex-col gap-3">
      <OnboardingFieldLabel label={field.label} required />
      <button
        type="button"
...
// 온보딩 2단계 화면 조립. 하단 진행 CTA는 시안에 없다 — 임의로 만들지 않는다(감사 MISSING)
export default function WikiOnboardingSourceStep({
  ...
  const [pollingField, backfillField, runTimeField] = scheduleFields;

  return (
    <div className="flex w-full flex-col gap-8 px-16 pt-5 pb-9">
      <OnboardingStepper steps={steps} currentStep={2} />
      <h1 className="text-display-xlarge text-text-normal-strong">{heading}</h1>

      <div className="bg-fill-normal-assistive border-line-normal-neutral flex flex-col gap-8 rounded-2xl border p-8">
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="border-line-normal-neutral bg-fill-normal-normal hover:bg-fill-normal-interaction-hover flex h-13.5 w-full cursor-pointer items-center gap-3 rounded-xl border px-3 transition-colors"
          >
        ...
        <div className="grid grid-cols-2 gap-x-6">
          {pollingField && <ScheduleTriggerField field={pollingField} onOpenRequest={onOpenScheduleField} />}
          {backfillField && <ScheduleTriggerField field={backfillField} onOpenRequest={onOpenScheduleField} />}
        </div>

        {runTimeField && (
          <div className="flex flex-col gap-3">
            <ScheduleTriggerField field={runTimeField} onOpenRequest={onOpenScheduleField} />
        ...
        <div className="bg-fill-primary-normal-neutral flex items-center gap-2 rounded-lg px-2 py-1.5">
          <IconInfoFilled className="text-icon-primary-assistive size-4.5 shrink-0" />
