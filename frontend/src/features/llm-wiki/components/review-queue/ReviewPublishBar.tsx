...
interface ReviewPublishBarProps {
  ...
  canReview?: boolean;
  /** 미판정 블록이 남으면 발행이 거부되므로 그때까지 누를 수 없다 */
  disabled?: boolean;
  onPublish: () => void;
}

/** 검토 큐 상세 하단의 고정 바 — "최종 내보내기" 단일 액션. */
export default function ReviewPublishBar({ canReview = true, disabled = false, onPublish }: ReviewPublishBarProps) {
  ...
  return (
    <div className="border-line-normal-neutral flex shrink-0 items-center justify-end border-t px-8 py-3">
      ...
      <Button variant="box-solid-primary" size="lg" className="h-10" disabled={disabled} onClick={onPublish}>
        최종 내보내기
      </Button>
    </div>
  );
