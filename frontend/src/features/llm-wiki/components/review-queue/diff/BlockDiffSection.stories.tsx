...
const meta = {
  ...
  tags: ['autodocs'],
  args: { onEditDocument: fn(), onApprove: fn(), onRevert: fn(), onDelete: fn() },
  parameters: {
    ...catchupParameters({
      level: 'composition',
      domain: 'llm-wiki',
      ...
      dataNotes: [
        ...
        '직접 수정 버튼의 실제 동작(본 페이지 이동 등)은 미정 — 콜백만 뚫려 있다(2026-08-07 사용자 결정). 블록 단위 편집 진입도 미정이라 카드에는 편집 버튼이 없다.',
      ],
...
export const Default: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    // 직접 수정은 섹션 헤더에 하나뿐이다 — 카드에서 빠졌으므로 getByRole 단일 매치가 그 자체로 가드다
