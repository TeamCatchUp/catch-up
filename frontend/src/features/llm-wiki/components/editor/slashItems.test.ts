...
describe('filterSlashItems', () => {
  ...
  it('대소문자를 가리지 않는다', () => {
    expect(filterSlashItems(SLASH_ITEMS, 'CODE')).toEqual(filterSlashItems(SLASH_ITEMS, 'code'));
  });

  it('앞뒤 공백을 무시한다', () => {
    expect(filterSlashItems(SLASH_ITEMS, '  목록  ')).toEqual(filterSlashItems(SLASH_ITEMS, '목록'));
  });
