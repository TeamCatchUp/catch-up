  ...
  return (
    <div className="flex w-full items-stretch gap-5">
      <div className="flex w-5.5 shrink-0 flex-col items-center gap-0.5">
        <Icon className="text-icon-normal-neutral h-5.5 w-5.5 shrink-0" aria-hidden />
        {!isLast && <span aria-hidden className="bg-line-normal-neutral w-px flex-1" />}
      </div>

      <div className="flex min-w-0 flex-1 flex-col gap-2 pb-3">
        <p className="text-body-xsmall text-text-normal-alternative flex h-5.5 items-center">{step.title}</p>
        {step.lines.map((line, idx) => (
          <p key={idx} className="text-body-xsmall text-text-normal-alternative wrap-break-word">
            {line}
          </p>
        ))}
      </div>
