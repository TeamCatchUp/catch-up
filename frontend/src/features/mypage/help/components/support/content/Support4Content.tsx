...
export const SUPPORT_4_SECTIONS = [
  { id: 'empty-result', title: '1. 검색 결과가 텅 비었나요?' },
  { id: 'wrong-answer', title: '2. 답변이 원하던 방향과 조금 빗나갔나요?' },
  { id: 'error-message', title: '3. 화면이 멈추거나 에러 메시지가 떴나요?' },
  { id: 'support-channel', title: '4. 그래도 해결되지 않는다면 (Support)' },
  ...
  return (
    <>
      ...
      <HelpArticleSection {...SUPPORT_4_SECTIONS[0]}>
        ...
        <HelpArticleSubsection title="혹시 '비공개'로 설정된 글인가요?">
          <HelpArticleText>
            <p>
              아무것도 나오지 않는다면, 오류가 아니라
              <br />
              Catch Up의 철저한 보안 원칙이 작동하고 있다는 뜻입니다.
            </p>
            <br />
            <p>
      ...
      <HelpArticleSection {...SUPPORT_4_SECTIONS[1]}>
        <HelpArticleSubsection title="질문의 폭을 조금만 더 좁혀주세요.">
          <HelpArticleText>
            <p>
              수많은 업무 기록에서 나에게 딱 맞는 하나의 실마리를 건져 올리려면 작은 단서가 필요합니다.
              <br />
        ...
        <HelpArticleText>
          <p>단어 몇 개만 더해주셔도, Catch Up은 훨씬 더 선명하고 정확한 답을 찾아냅니다.</p>
        </HelpArticleText>
      </HelpArticleSection>

      <HelpArticleSection {...SUPPORT_4_SECTIONS[2]}>
        <HelpArticleSubsection title="새로고침을 한 번 해주세요.">
          <HelpArticleText>
            <p>
              사내 네트워크 환경이나 VPN 상태에 따라 연결이 일시적으로 불안정해질 수 있습니다.
              <br />
              대부분의 일시적인 엇갈림은 새로고침(F5) 한 번이면 해결됩니다.
            </p>
          </HelpArticleText>
        </HelpArticleSubsection>

        <HelpArticleSubsection title="연동 상태나 사내 보안 환경에 작은 변화가 생겼는지 확인해 주세요.">
          <HelpArticleText>
            <p>
        ...
        <HelpExampleBox badgeLabel="경우">
          <ul className="list-disc space-y-3 pl-6">
            <li>사내 보안 시스템의 권한 정보가 갱신되거나 세션이 만료된 경우</li>
            <li>관리자에 의해 원본 도구에 설치된 Catch Up 앱이 일시적으로 삭제되거나 권한이 변경된 경우</li>
          </ul>
        </HelpExampleBox>
      ...
      <HelpArticleSection {...SUPPORT_4_SECTIONS[3]}>
        <HelpArticleSubsection title="가장 가까운 곳에서, 가장 빠른 방법으로 돕겠습니다.">
          <HelpArticleText>
            <p>
        ...
        <HelpArticleSubsection title="격식은 생략하셔도 좋습니다.">
          <HelpArticleText>
            <p>
              에러 화면 캡처 한 장, 혹은 로그 한 줄만 남겨주셔도 충분합니다.
              <br />
              업무 맥락을 깨지 않고 그 안에서 바로 해답을 찾아드리겠습니다.
            </p>
          </HelpArticleText>
        </HelpArticleSubsection>

        <HelpArticleSubsection title="여러분의 질문이 동료의 해답이 됩니다.">
          <HelpArticleText>
            <p>
