...
1. `codebase` (소스 코드 검색):
   - 특정 기능의 구현 방식, 클래스/함수 정의, 프로젝트 구조, 코드 로직에 대한 질문.
   - 예: "auth_middleware는 어떻게 구현되어 있어?", "User 모델 정의 보여줘", "메인 로직이 어디야?"
...
4. `chitchat` (검색 불필요):
   - 개발 업무와 무관한 가벼운 인사, 감사 표현, AI의 정체성 질문.
   - 예: "안녕", "고생했어", "너는 누구니?", "점심 메뉴 추천해줘"

[주의 사항]
- 질문이 모호하여 코드와 히스토리가 모두 필요해 보일 때는, 가장 핵심적인 의도 하나를 선택하세요.
- "구현(How)"은 `codebase`, "이유(Why)"는 `pr_history`, "문제(Problem)"는 `issue_tracker`로 분류하는 것이 원칙입니다.
