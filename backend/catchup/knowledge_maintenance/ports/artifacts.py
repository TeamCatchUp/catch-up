...
class ArtifactRepository(Protocol):
    ...
    def lock_owner_user_ids(
        ...
        *,
        ...
        검토 확정이 담당자 규칙을 자기 transaction 안에서 다시 볼 때 쓴다.
        라우터가 다른 세션으로 미리 읽은 명단은 결정이 확정되기까지 사이에
        바뀔 수 있어, 그 읽기만 믿으면 담당자가 지정된 직후에 도착한 남의
        결정이 그대로 실린다.

        문서 행을 잠그는 것이 요점이다. 담당자를 지정·해제하는 경로도 같은
        행을 잠그므로, 명단을 바꾸는 일과 결정을 확정하는 일이 그 행 하나를
        두고 줄을 선다. 명단 행에만 잠금을 걸면 아직 없는 행은 잠글 수
        없어 새 담당자가 그 사이로 끼어든다.

        문서 행은 이 저장소에서 가장 바깥 잠금이므로, 변경안 행을 함께 잡는
        경로에서 불러도 잡는 차례가 뒤집히지 않는다.
        """
        ...
    ...
    def add_owner_if_absent(
        ...
        *,
        ...
        같은 문서를 동시에 확정하면 두 transaction이 같은 담당자 행을
        넣으려 들 수 있다. (문서, 사용자) 유일 제약 위반만 무동작으로
        흡수하고, 다른 무결성 위반은 그대로 올려 보낸다. 전부 삼키면 외래
        키가 막은 잘못된 부여가 조용히 사라진다.
        """
        ...
