...
CASES: tuple[EvalCase, ...] = (
    ...
    EvalCase(
        # 실제 파이프라인 발췌(Observation 앞 300자)를 그대로 쓴 어려운
        # 양성이다. 후보 일부의 발췌가 다른 시스템(아틀라스·머큐리) 얘기
        # 중심이라 judge가 흔들린다. 베이스라인에서 알려진 실패 케이스로,
        # excerpt 개선(발췌를 후보 언급 주변으로 앵커링) 튜닝의 과녁이다.
        key="캐치업(노이즈 발췌)",
        kind="hard-positive",
        expected_same=True,
        members=(
