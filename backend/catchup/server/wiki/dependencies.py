...
검수는 위키 역할 위에 서는 표면이므로, 아래층인 위키 패키지가 공통 재료를
갖고 검수가 그것을 가져다 쓴다. 반대 방향으로 두면
...
def resolve_member_workspace(
    ...
    역할 게이트가 없다는 것이 이 문의 요점이다. 위키 표면은 전부
    "구성원이면 누구나 서고, 대상마다 자격을 다시 본다"는 모양이다. 여기서
    역할로 미리 막으면, 아직 채널이 하나도 없는 사람은 어떤 역할도 없어
    채널을 만들 문 앞에서 막힌다.
