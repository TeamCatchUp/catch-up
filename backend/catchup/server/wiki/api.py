...
def delete_folder(
    ...
    폴더를 지우면 그 폴더의 문서와 정의는 채널 루트(folder_id NULL)로
    옮겨진다. 두 FK가 모두 ON DELETE SET NULL이라 DB가 옮긴다. 폴더는
    묶어 보는 자리일 뿐이고 문서와 정의는 채널에 속하므로, 자리를 없앤다고
    내용까지 사라지거나 삭제가 막혀서는 안 된다.

    Raises:
        HTTPException: 채널·폴더가 없으면 404, 관리자가 아니면 403을 던진다.
    """
    _require_channel_admin(db, channel_id=channel_id, context=context)
    folder = _load_folder(db, folder_id=folder_id, channel_id=channel_id)
