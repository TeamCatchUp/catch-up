...
위키 역할(채널 관리자·문서 담당자·전역 ADMIN)을 둘 다 통과해야 하고, 주지
않으면 소속이 하나일 때만 그것으로 정한다. 여러 곳에 속한 사람에게 임의로
하나를 골라 주면 남의 workspace 문서를 자기 것으로 착각한 채 결정할 자리가
생긴다.
...
def resolve_reviewer_workspace(
    ...
    위키 역할을 모두 통과해야 한다 — 소속만 있으면 403(NOT_REVIEWER),
    소속이 없으면 403(NOT_MEMBER).
