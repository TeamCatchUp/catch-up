...
def cost_query_params(
    start_time: datetime = Query(
        description="집계 시작 시간 (ISO 8601)",
    ),
    end_time: datetime | None = Query(
        default=None,
        description="집계 종료 시간 (ISO 8601). 없으면 현재 시간 기준.",
    ),
) -> CostQueryParam:
    resolved_end = end_time or datetime.now(UTC)
    
    if (resolved_end - start_time).days > MAX_RANGE_DAYS:
    ...
    return CostQueryParam(start_time=start_time, end_time=resolved_end)
