    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        if self.tool_filters:
            filter_clause = "AND e.cmetadata ->> 'source' = ANY(:filters)"
            params["filters"] = [f.value for f in self.tool_filters]
...
class PGVectorService(BaseVectorDbService):
    ...
    def _ensure_index(self):
        ...
        JSONB 타입의 cmetadata 컬럼의 'source' 필드를 필터링 하기 위한 B-Tree 인덱스를 생성한다.
