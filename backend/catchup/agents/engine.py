...
class ExecutionService:
    def __init__(self, llm: BaseChatModel) -> None:
        self._llm = llm
        # spec_id(DB PK) → CompiledStateGraph. LRU eviction으로 상한 유지한다.
        self._graph_cache: OrderedDict[int, CompiledStateGraph] = OrderedDict()
    ...
    def _get_graph(self, spec_id: int, spec: AgentSpec) -> CompiledStateGraph:
        """spec_id 기준으로 컴파일된 그래프를 LRU 캐시에서 반환한다."""
        if spec_id in self._graph_cache:
            self._graph_cache.move_to_end(spec_id)
            return self._graph_cache[spec_id]
        if len(self._graph_cache) >= _GRAPH_CACHE_MAX_SIZE:
