...
검증 대상:
  - @action에 선언된 default_failure_policy / default_max_retry가 ActionSpec에 저장된다.
...
def _make_tool(
    ...
    class _Tool(BaseTool):
        ...
        async def my_action(self, query: str) -> dict:
            return {}
...
def test_action_spec_stores_default_failure_policy():
    ...
    tool = _make_tool(failure_policy=FailurePolicy.SKIP)
    spec = tool.get_action_spec("my_action")
    assert spec.default_failure_policy == FailurePolicy.SKIP
...
def test_action_spec_stores_explicit_default_max_retry():
    ...
    tool = _make_tool(failure_policy=FailurePolicy.CONTINUE, max_retry=3)
    spec = tool.get_action_spec("my_action")
...
def test_base_tool_schema_exposes_default_policies():
    ...
    tool = _make_tool(failure_policy=FailurePolicy.STOP, max_retry=2)
    schema = tool.schema()

    action_schema = schema["actions"]["my_action"]
    assert action_schema["default_failure_policy"] == "stop"
