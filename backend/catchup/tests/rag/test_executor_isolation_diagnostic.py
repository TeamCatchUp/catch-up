...
class TestExecutorIsolationDiagnostic(IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self._original_llm_executor = rag_executors.llm_executor
        self._test_llm_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="llm-test")
        rag_executors.llm_executor = self._test_llm_executor

        # Rerank 테스트용 executor 설정
        self._original_rerank_executor = rag_executors.rerank_executor
        self._test_rerank_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="rerank-test")
        rag_executors.rerank_executor = self._test_rerank_executor

    def tearDown(self):
        self._test_llm_executor.shutdown(wait=False)
        rag_executors.llm_executor = self._original_llm_executor
        
        self._test_rerank_executor.shutdown(wait=False)
        rag_executors.rerank_executor = self._original_rerank_executor

    async def test_isolated_chat_bedrock_uses_llm_executor(self):
        """IsolatedChatBedrock._agenerate 호출 시 rag_executors.llm_executor가 사용되는지 확인"""
        ...
        self.assertIn(rag_executors.llm_executor, captured_executors, "IsolatedChatBedrock은 rag_executors.llm_executor를 사용해야 합니다.")

    async def test_reranker_service_uses_rerank_executor(self):
        """AwsBedrockRerankService.rerank 호출 시 rag_executors.rerank_executor가 사용되는지 확인"""
        ...
        self.assertIn(rag_executors.rerank_executor, captured_executors, "AwsBedrockRerankService는 rag_executors.rerank_executor를 사용해야 합니다.")
