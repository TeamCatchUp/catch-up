...
class GithubPrV2BackfillService:
        ...
        *,
        ...
        async def _process_target(target) -> None:
            ...
            try:
                ...
                failed_ids: list[str] = []
                backfill_count = 0
                seed_cursor: GithubPrV1SeedCursor | None = None
                seed_page_index = 0
                chunk_index = 0
                ...
                while True:
                    ...
                    seed_cursor = GithubPrV1SeedCursor(
    ...
    def _fetch_candidate_seed_page_for_target_sync(
        ...
        cursor: GithubPrV1SeedCursor | None,
        limit: int,
    ) -> list[GithubPrV1Seed]:
    ...
    def _mark_processing_sync(self, target: GithubPrV1Target) -> datetime | None:
        with self._session_factory() as db:
            result = db.execute(
    ...
    def _mark_finished_sync(
        ...
        *,
        ...
        if rowcount == 0:
            logger.warning(
                "github_pr_v2_backfill_target_finish_update_missed",
                **_target_log_context(target),
                state=state,
                backfill_count=backfill_count,
                failed_count=len(failed_ids),
...
def _github_pr_needs_backfill_expr() -> str:
    return f"""
                    (
    ...
    return text(
        ...
        candidates AS (
            SELECT
                v1_pr.scope_id,
                v1_pr.target_id,
                {_github_pr_needs_backfill_expr()} AS needs_backfill
            FROM v1_pr
