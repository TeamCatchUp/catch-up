...
class JiraEpicMetadata(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class JiraEpicVectorRecord(BaseModel):
    """v2 vector-store row contract for a Jira epic."""
    ...
    def langchain_metadata(self) -> dict[str, Any]:
        return {
            "jira_epic": self.jira_epic.model_dump(
                mode="json",
                exclude_none=False,
            )
        }
    ...
    def to_document(self) -> Document:
        ...
        metadata["jira_epic"] = values["langchain_metadata"]["jira_epic"]
        return Document(
            id=self.langchain_id,
            page_content=self.content,
            metadata=metadata,
