...
async def rerank_node(state: AgentState, rerank_service: BaseRerankService):
    ...
    try:
        ...
        logger.info("semaphore_acquiring", semaphore="rerank", doc_count=len(retrieved_docs))
        async with rag_semaphores.rerank:
            ...
            logger.info("rerank_invoke_completed", elapsed=round(time.perf_counter() - t_rerank, 3))
